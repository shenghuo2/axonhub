...
type AnnouncementQuery struct {
	...
	loadTotal        []func(context.Context, []*Announcement) error
...
func (_q *AnnouncementQuery) QueryAPIKeys() *APIKeyQuery {
...
func (_q *AnnouncementQuery) WithAPIKeys(opts ...func(*APIKeyQuery)) *AnnouncementQuery {
	query := (&APIKeyClient{config: _q.config}).Query()
...
func (_q *AnnouncementQuery) prepareQuery(ctx context.Context) error {
	...
	if err := announcement.Policy.EvalQuery(ctx, _q); err != nil {
		return err
	}
	return nil
}
...
func (_q *AnnouncementQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Announcement, error) {
	...
	if query := _q.withAPIKeys; query != nil {
		if err := _q.loadAPIKeys(ctx, query, nodes,
			func(n *Announcement) { n.Edges.APIKeys = []*APIKey{} },
			func(n *Announcement, e *APIKey) { n.Edges.APIKeys = append(n.Edges.APIKeys, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedAPIKeys {
		if err := _q.loadAPIKeys(ctx, query, nodes,
			func(n *Announcement) { n.appendNamedAPIKeys(name) },
			func(n *Announcement, e *APIKey) { n.appendNamedAPIKeys(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *AnnouncementQuery) loadAPIKeys(ctx context.Context, query *APIKeyQuery, nodes []*Announcement, init func(*Announcement), assign func(*Announcement, *APIKey)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Announcement)
	nids := make(map[int]map[*Announcement]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(announcement.APIKeysTable)
		s.Join(joinT).On(s.C(apikey.FieldID), joinT.C(announcement.APIKeysPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(announcement.APIKeysPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(announcement.APIKeysPrimaryKey[0]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*Announcement]struct{}{byID[outValue]: {}}
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "api_keys" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *AnnouncementQuery) Modify(modifiers ...func(s *sql.Selector)) *AnnouncementSelect {
...
func (_q *AnnouncementQuery) WithNamedAPIKeys(name string, opts ...func(*APIKeyQuery)) *AnnouncementQuery {
	query := (&APIKeyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// Modify adds a query modifier for attaching custom logic to queries.
func (_s *AnnouncementSelect) Modify(modifiers ...func(s *sql.Selector)) *AnnouncementSelect {
